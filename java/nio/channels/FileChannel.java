    ...
    implements SeekableByteChannel, GatheringByteChannel, ScatteringByteChannel
{
     ...
     *
     * 将此通道的字节写入ByteBuffer，从position-limit
     */
     ...
     *
     * 将此通道的字节写入ByteBuffer[offset]-ByteBuffer[offset+length-1]
     * 每个从position-limit，直到channel读完或者ByteBuffer[]写完
     */
     ...
     * interface.  </p>
     *
     * 将ByteBuffer[offset]-ByteBuffer[offset+length-1]写入到管道
     * 每个从position到limit
     */
     ...
     *          If some other I/O error occurs
     *
     *          不会修改该channel的position
     *          如果position>=size，则不会传输任何字节
     *          传输到target，从position写入
     */
     ...
     *          If some other I/O error occurs
     *
     *          从src中读取字节写入到此通道的position->position+count
     *
     */
     ...
     *
     *  将通道的文件直接映射到MappedByteBuffer，position=0，limit=capacity =size
     *  一旦被影射，就不依赖此channel，依赖于操作系统
     *
     *  param ：position和size都是指的文件，从position--->position+size开始映射
     */
     ...
     *
     *  从position到position+size锁定，shared表示是否为共享锁
     *  如果为共享锁，则此通道必须为可读
     *  如果不是共享锁，则此通道我必须为可写
     *  返回对应锁的对象
     *  进程锁
