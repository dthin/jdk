    ...
    extends OutputStream implements ObjectOutput, ObjectStreamConstants
{
     ...
     *
     * 如果为true，则使用子类的writeObjectOverride方法
     * 默认为false
     *
     * */
     ...
     *
     * 在上调到类定义的writeObject方法的上下文中;
       保持当前正在序列化的对象和当前类的描述符。
       没有在writeObject upcall时空。
     */
        ...
        extends OutputStream implements DataOutput
    {
         ...
         * mode.
         *
         * 将块数据模式设置为给定模式（true == on，false == off）并返回上一个模式值。
         * 如果新模式与旧模式相同，则不执行任何操作。
         * 如果新模式与旧模式不同，则在切换到新模式之前，将刷新任何缓冲数据。
         */
         ...
         * original byte array).
         * 从给定的数组写入指定的字节值范围。
         * 如果copy为true，则将值写入中间缓冲区，然后将其写入底层流（以避免暴露对原始字节数组的引用）。
         */
            ...
            throws IOException
        {
            ...
            while (len > 0) {
                ...
                if (len >= MAX_BLOCK_SIZE && !copy && pos == 0) {
                    ...
                    /**如果要写入数据大于缓冲区，且copy=false，缓冲区为空
                     * 直接写入底层流
                     * */
    ...
    private static class HandleTable {
         ...
         * no mapping found.
         * 查找并返回与给定对象关联的句柄，如果没有找到映射，则返回-1。
         */
         ...
         *
         * 从hash值找到objs中的位置，根据他在objs的位置，从next找到下一个值
         *
         * 每次用新值覆盖spine[index]，同时在next[handle]加入旧值
         */
